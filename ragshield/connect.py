"""Native cognis-connect emit for ragshield — forward findings to any platform.

Maps ragshield's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    ragshield ... --format json | ragshield-emit --to stix
    ragshield-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('message', '')
        out['severity'] = rec.get('severity', 'info')
        out['type'] = rec.get('detector', '')
