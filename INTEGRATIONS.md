...
**ragshield** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `ragshield` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
ragshield ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
ragshield ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `ragshield` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="ragshield") for rec in records]   # records = your JSON output
