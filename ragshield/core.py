...
TOOL_NAME = "RAGSHIELD"
TOOL_VERSION = "0.1.0"
RULES = [{'id': 'RS-DUP-001', 'severity': 'medium', 'weight': 2.0, 'title': 'DUPLICATE_AUTHORITATIVE_CHUNK', 'when': {'duplicate_count': '^[1-9]'}, 'remediation': 'Dedupe ingestion; verify source authority.'}, {'id': 'RS-TRIG-001', 'severity': 'high', 'weight': 2.5, 'title': 'BACKDOOR_TRIGGER', 'when': {'contains_trigger': True}, 'remediation': 'Quarantine document; rebuild embeddings without it.'}, {'id': 'RS-OUT-001', 'severity': 'medium', 'weight': 2.0, 'title': 'EMBEDDING_OUTLIER', 'when': {'outlier': True}, 'remediation': 'Inspect; outliers can be intentional injection vectors.'}]
        ...
        for rule in RULES:
            if _match(item, rule):
                result.add(Finding(
                    ...
                    location=str(target),
                    remediation=rule.get("remediation",""), category="rag-corpus",
                    metadata=item,
                ))
